/*
 * Took from: https://github.com/obsproject/obs-websocket/blob/master/lib/obs-websocket-api.h
...
#include <obs.h>
#include <util/dstr.h> // obs_data_get_json needs this? No, but general utility.
...
static proc_handler_t *_ph = NULL;
...
static inline proc_handler_t *obs_websocket_get_ph(void)
{
	...
	// assert(global_ph != NULL); // Removed assert for cleaner plugin behavior
...
static inline bool obs_websocket_vendor_run_simple_proc(obs_websocket_vendor vendor, const char *proc_name, calldata_t *cd)
{
	...
	if (!vendor || !proc_name || !cd) // removed strlen check for simplicity
		return false;
