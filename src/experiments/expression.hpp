...
/**
 * expression.hpp - Expression Engine Aggregate Header
 * 
 * Provides dynamic value types, expression parsing and evaluation.
 * - ExpressionValue: Dynamic typed values
...
public:
    ...
    bool Set(const std::string& name, ExpressionValue value) {
        for (auto it = scopes_.rbegin(); it != scopes_.rend(); ++it) {
            ...
            if (var_it != scope.end()) {
                if (var_it->second.is_const) return false;
    ...
    void Clear() { scopes_.clear(); scopes_.emplace_back(); }
